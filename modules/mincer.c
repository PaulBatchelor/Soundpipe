...
int sp_mincer_destroy(sp_mincer **p)
{
    ...
    sp_auxdata_free(&pp->fwin[0]);
    sp_auxdata_free(&pp->bwin[0]);
    sp_auxdata_free(&pp->prev[0]);
    sp_auxdata_free(&pp->framecount[0]);
    sp_auxdata_free(&pp->outframe[0]);
...
int sp_mincer_init(sp_data *sp, sp_mincer *p, sp_ftbl *ft)
{
    ...
    sp_auxdata_alloc(&p->fwin[i], size);
    sp_auxdata_alloc(&p->bwin[i], size);
    sp_auxdata_alloc(&p->prev[i], size);
    size = decim*sizeof(int);
    sp_auxdata_alloc(&p->framecount[i], size);
    {
      int k=0;
      for (k=0; k < decim; k++) {
        ((int *)(p->framecount[i].ptr))[k] = k*N;
      }
    }
    size = decim*sizeof(SPFLOAT)*N;
    sp_auxdata_alloc(&p->outframe[i], size);
    }
    ...
    //for (n=offset; n < nsmps; n++) {
      ...
      if (cnt == hsize) {
        ...
        */
        ...
        //for (j = 0; j < nchans; j++) {
          pos = spos;
          bwin = (SPFLOAT *) p->bwin[0].ptr;
          fwin = (SPFLOAT *) p->fwin[0].ptr;
          prev = (SPFLOAT *)p->prev[0].ptr;
          framecnt  = (int *)p->framecount[0].ptr;
          outframe= (SPFLOAT *) p->outframe[0].ptr;
          ...
          */
          for (i=0; i < N; i++) {
            ...
            if(post+nchans <  size)
              insig = tab[post] + frac*(tab[post+nchans] - tab[post]);
            else insig = tab[post];

            fwin[i] = insig * win[i]; /* window it */
            /* back windo, bwin */
