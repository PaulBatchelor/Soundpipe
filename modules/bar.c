...
int sp_bar_init(sp_data *sp, sp_bar *p)
{
    ...
    SPFLOAT T30 = p->iT30;   /* ~=5.0; 30 db decay time (s) */
...
int sp_bar_compute(sp_data *sp, sp_bar *p, SPFLOAT *in, SPFLOAT *out)
{
    ...
    int bcL = (int)lrintf((SPFLOAT)p->bcL);    /*  boundary condition pair */
    int bcR = (int)lrintf((SPFLOAT)p->bcR);    /*  1: clamped, 2: pivoting, 3: free */
    SPFLOAT SINNW = sin(xofreq*step); /* these are to calculate sin/cos by */
    SPFLOAT COSNW = cos(xofreq*step); /* formula rather than many calls    */
    SPFLOAT SIN1W = sin(xofreq);      /* Wins in ksmps>4 */
    SPFLOAT COS1W = cos(xofreq);

    if ((bcL|bcR)&(~3) && (bcL|bcR)!=0) {
        fprintf(stderr, "sp_bar: Ends must be clamped(1), pivoting(2), or free(3)\n");
        return SP_NOT_OK;
    }
    //for (n = offset; n < nsmps; n++) {
      ...
      if (first == 0) {
        p->first = first = 1;
        for (rr = 0; rr < N; rr++) {
          if (fabs(rr/(SPFLOAT)N - p->ipos) <= p->iwid) {
            w[rr+2] += (1.0/sp->sr)*(p->ivel)*0.5*
                (1.0+cos(M_PI*fabs(rr/(SPFLOAT)N-(p->ipos))/(p->iwid)));
          }
        }
      }
      {
