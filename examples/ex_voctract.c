...
void process(sp_data *sp, void *udata) {
    UserData *ud = udata;
    SPFLOAT osc = 0, voc = 0, square = 0;

    sp_blsquare_compute(sp, ud->square, NULL, &square);
    sp_osc_compute(sp, ud->osc, NULL, &osc);
    if(sp_voc_get_counter(ud->voc) == 0) {
        osc = 12 + 16 * (0.5 * (osc + 1));
        sp_voc_set_tongue_shape(ud->voc, osc, 2.9);
    }
    sp_voc_tract_compute(sp, ud->voc, &square, &voc);
    sp->out[0] = voc;
}
